...
import { ArrowLeft, ChevronLeft, ChevronRight, Heart, MapPin, MessageCircle, Pencil, RotateCcw, Send, ThumbsDown, ThumbsUp, Trash2, Users, Wine, X } from 'lucide-react';
import { useEffect, useMemo, useRef, useState } from 'react';
import { addBarComment, deleteBarRanking, observeBarSocial, saveBarRanking, setBarReaction, updateBarPostText, type BarRanking, type BarSocialData, type SignedInUser } from './firebase';
...
export default function BarSwipeApp({ user, onHome, onSignIn }: Props) {
  ...
  return <main className="bars-shell">
    ...
    </section> : <Community rankings={community} social={social} user={user} onDeleted={(barId) => setSocial((data) => ({ ...data, rankings: data.rankings.filter((item) => item.uid !== user.uid || item.barId !== barId) }))} />}
  </main>;
}

function Community({ rankings, social, user, onDeleted }: { rankings: BarRanking[]; social: BarSocialData; user: SignedInUser; onDeleted: (barId: string) => void }) {
  const [filter, setFilter] = useState('All'); const neighborhoods = ['All', 'Gold Coast', 'Old Town', 'Lincoln Park', 'Wrigleyville'];
  const [editing, setEditing] = useState(''); const [editText, setEditText] = useState(''); const [drafts, setDrafts] = useState<Record<string, string>>({}); const [working, setWorking] = useState(''); const [notice, setNotice] = useState('');
  ...
  return <section className="community-view"><header><div><p className="bars-kicker">Live community feed</p><h1>Who ranked what</h1></div><span>{visible.length} posts</span></header><div className="neighborhood-filters">{neighborhoods.map((item) => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div><div className="ranking-list social-feed">{visible.length ? visible.map((ranking) => {
    ...
    return <article className="social-post" key={postKey}><div className="post-head"><span className="rank-avatar">{ranking.displayName.slice(0,1).toUpperCase()}</span><div><strong>{ranking.displayName}</strong><p>{ranking.choice === 'like' ? 'liked' : 'passed on'} <b>{ranking.barName}</b></p><small>{ranking.neighborhood}</small></div>{ranking.uid === user.uid && <button className="delete-match" disabled={working === postKey} aria-label={`Delete your match with ${ranking.barName}`} onClick={async () => { if (!window.confirm(`Delete your match with ${ranking.barName}? You can swipe on it again afterward.`)) return; setWorking(postKey); setNotice(''); try { await deleteBarRanking(user, ranking.barId); onDeleted(ranking.barId); } catch { setNotice('Could not delete that match. Please try again.'); } finally { setWorking(''); } }}><Trash2 size={16} /></button>}<span className={`choice-badge ${ranking.choice}`}>{ranking.choice === 'like' ? <Heart size={16} fill="currentColor" /> : <X size={16} />}{ranking.choice}</span></div>
  ...
  }) : <div className="rank-empty"><RotateCcw size={32} /><h2>No posts here yet.</h2><p>Be the first to swipe on a bar.</p></div>}</div>{notice && <p className="bars-error" role="status">{notice}</p>}</section>;
}
